function [MDP] = Z_spm_MDP_VB_X(MDP,OPTIONS)
% active inference and learning using variational Bayes (factorised)
% FORMAT [MDP] = spm_MDP_VB_X_EpistemicLearning(MDP,OPTIONS)
%
...
% Karl Friston
% $Id: spm_MDP_VB_X_EpistemicLearning.m 6902 2016-10-08 19:28:25Z karl $
...
if length(MDP) > 1
    ...
    for i = 1:length(MDP)
        ...
        % solve this trial
        %------------------------------------------------------------------
        OUT(i) = Z_spm_MDP_VB_X(MDP(i),OPTS);
...
% precision defaults
%--------------------------------------------------------------------------
try ambiguity = MDP.ambiguity; catch, ambiguity = true;   end
try curiosity = MDP.curiosity; catch, curiosity = true;   end
...
for t = 1:T
    ...
    if isfield(MDP,'link')
        ...
        %------------------------------------------------------------------
        MDP.mdp(t) = spm_MDP_VB_X_EpistemicLearning(mdp);
...
for t = 1:T
    ...
    if isfield(MDP,'a')
        for g = 1:Ng
            da     = sparse(o(g,t),1,1,No(g),1); % current observation
            for  f = 1:Nf
                da = spm_cross(da,X{f}(:,t));
            end
            da       = da.*(MDP.a{g} > 0); % find out where learning can take place
            MDP.a{g} = MDP.a{g} + da*eta;  % eta something like a learning rate
...
%         MDP.d{f}(i) = MDP.d{f}(i) + X{f}(i,1);
        MDP.d{f}(i) = MDP.d{f}(i) + X{f}(i,1)*eta;  %PS
    end
end
